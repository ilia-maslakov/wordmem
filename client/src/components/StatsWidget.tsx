...
export const StatsWidget: React.FC<StatsWidgetProps> = ({ stats, hasWords }) => {

    const total = (stats.totalCount ?? 1) + pairsCount * 1.2

    const percent = stats.turn > 0 ? Math.round((stats.correctCount / stats.turn) * 100) : 0
    ...
    return (
        <div className="max-w-xl mx-auto ">
            <div className="p-4 bg-white rounded-lg text-center border gap-4">
                ...
                <div className="text-lg text-gray-600">
                    Правильных:{" "}
                    <span className="font-semibold text-green-600">{stats.correctCount} / {percent}%</span>
                </div>
            </div>
        </div>
    )
