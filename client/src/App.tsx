...
import {ButtonVariant} from "@/components/ui/button.tsx";
...
export const App = () => {
  ...
  const remainingPairs = useMemo(
    () => pairs.filter((p) => p.shows > 0).length,
    [pairs],
  );

  const hasWords = useMemo(() => remainingPairs > 0, [remainingPairs]);

  // useEffect(() => {
  //   console.log(`О1сталось пар: ${remainingPairs}`);
  //   setHasWords(remainingPairs > 0);
  // }, [remainingPairs]);
    ...
    <div>
      ...
      {!hasWords && (
        <div className="flex justify-center">
          <ButtonVariant
            className="px-10 py-2 text-base mt-1"
            onClick={reloadGame}
          >
            Начать заново
          </ButtonVariant>
